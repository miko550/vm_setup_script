...
#Step 2
elseif ((Get-WmiObject -Namespace root\cimv2 -Class Win32_ComputerSystem).Domain -ne "TRAINING.local") {
    #Add domain name to auto login
    Set-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name 'DefaultDomainName' -Type String -Value "TRAINING";

    #Create TRAINING.local domain. Server will restart after domain is created
    ...
    $userobj = $(try {Get-ADUser "trainingAdmin"} catch {$Null});
    if ($userobj -eq $Null) {
        ...
        #Modify scheduled task to complete as trainingAdmin user in elevated context
        $trigger = New-ScheduledTaskTrigger -AtLogon -User 'trainingAdmin';
        Set-ScheduledTask -TaskName 'SetupDC' -User 'trainingAdmin' -Trigger $trigger;
        Write-Host "[i] Modified SetupDC scheduled task to complete as trainingAdmin";

        #Changing autologon credentials to new user
        Set-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name 'DefaultUserName' -Type String -Value "trainingAdmin";
        Set-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -Name 'DefaultPassword' -Type String -Value "Passw0rd!";
        Write-Host "[i] Changed autologon creds to TRAINING\trainingAdmin";
